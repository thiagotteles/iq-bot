...
begin
	...
	begin
		...
		if @dirWin = 'd'
			insert into mhi5m values (@par, @data, @hora, 'skip', 0)
		else if @cor = @dirWin
			insert into mhi5m values (@par, @data, @hora, 'win', 0)
		else
		begin
			select @cor = cor
			from velaM5
			...
			if @cor = @dirWin
				insert into mhi5m values (@par, @data, @hora, 'win', 1)
			else
			begin
				select @cor = cor
				from velaM5
				...
				if @cor = @dirWin
					insert into mhi5m values (@par, @data, @hora, 'win', 2)
				else
					insert into mhi5m values (@par, @data, @hora, 'loss', 2)
			end
